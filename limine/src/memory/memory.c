...
#define IS_ALIGNED(x) ((((uint64_t)(x)) & (PAGESIZE-1)) == 0)  // macro para ver si la página que le vamos a pasar está alineada (0, 4096, 8192, ...) a tamaño de página. Restar 1 al tamaño y hacer AND, y da 0, no está alineado (hay un resto) 
#define ALIGN_ADDR(x) (((PAGESIZE-1)&(x))? ((x+PAGESIZE) & ~(PAGESIZE-1)) : (x))   // dirección alineada más cercana posible.

#define ADDR_TO_PAGE(x) ((uint64_t)(x) >> 12)  // desplazamento 12 veces = dividir entre 4096
...
void free_page(void* addr) {
    ...
    uint64_t normalized_address = (addr_uint - memory->first_available_page_addr);

    if (addr_uint < memory->first_available_page_addr)
        panic("Page below active range");

    if (GET_PAGE_BIT(normalized_address >> 12) == 0)
        panic("page was not locked");

    unlock_page((void*)normalized_address);

    if (GET_PAGE_BIT((normalized_address >> 12)) == 1) {
...
void free_contiguous_pages(void* address, uint64_t num) {
    ...
    for (uint64_t i = 0; i < num; i++) {
        if (GET_PAGE_BIT((normalized_address >> 12) + i) == 0)
            panic("Page was not locked!");
        unlock_page((void*)(normalized_address + (i << 12)));
    }
...
int init_memory() {
    ...
    memory->bitfield = (uint8_t*)ALIGN_ADDR(first_available_address + sizeof(struct system_memory*));   // siguiente página para el bitfield
    ...
    uint64_t used_space = (bitfield_entries + sizeof(struct system_memory) + (3 << 12)) >> 12;  // tres páginas. La reserva no es de bits, así que dividimos por el tamaño de página
